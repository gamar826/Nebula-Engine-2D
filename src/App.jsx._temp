...
import { 
  ...
  ImagePlus, Globe, List, Sigma, FunctionSquare, Calculator, Braces, BookOpen,
  Terminal, Puzzle, Workflow, ArrowBigRight, ArrowBigLeft, Percent, ChevronUp,
  Subscript, Superscript, Parentheses, Brackets, CurlyBraces, Asterisk
} from 'lucide-react';
...
const INITIAL_PROJECT = {
  ...
  settings: { 
    ...
    gameFrame: { enabled: true, showInEditor: true, color: '#3b82f6', opacity: 0.3, offsetX: 0, offsetY: 0 },
    globalVariables: []
  },
  scenes: [
    {
...
const loadSavedState = () => {
  try {
    ...
    if (saved) {
      ...
      if (!parsed.project.settings.gameFrame) parsed.project.settings.gameFrame = { enabled: true, showInEditor: true, color: '#3b82f6', opacity: 0.3, offsetX: 0, offsetY: 0 };
      if (!parsed.project.settings.globalVariables) parsed.project.settings.globalVariables = [];
      ...
      parsed.project.scripts.forEach(s => {
        if (!s.localVariables) s.localVariables = [];
      });
...
function engineReducer(state, action) {
  ...
  switch (action.type) {
    ...
    case 'OPEN_EXPRESSION_EDITOR':
      return {
        ...
        editor: {
          ...
          expressionEditor: {
            isOpen: true,
            targetNodeId: action.payload.nodeId,
            targetField: action.payload.field,
            expression: action.payload.initialExpression || { type: 'LITERAL', value: 0, children: [] }
          }
        }
      };
    case 'CLOSE_EXPRESSION_EDITOR':
      return {
        ...state,
        editor: {
          ...state.editor,
    ...
    case 'SAVE_EXPRESSION': {
      const activeScriptId = state.editor.activeEditingScriptId;
      if (!activeScriptId) return state;

      const updatedScripts = state.project.scripts.map(s => {
        if (s.id === activeScriptId) {
          return {
            ...s,
            nodes: s.nodes.map(n => {
              if (n.id === state.editor.expressionEditor.targetNodeId) {
                return { ...n, params: { ...n.params, [state.editor.expressionEditor.targetField]: action.payload.expression } };
              }
              return n;
            })
          };
      ...
      return {
        ...state,
        project: { ...state.project, scripts: updatedScripts },
        editor: {
          ...state.editor,
          expressionEditor: { isOpen: false, targetNodeId: null, targetField: null, expression: null }
        }
    ...
    case 'ADD_GLOBAL_VARIABLE': {
      const newVar = {
        id: generateId(),
        name: action.payload.name || 'var_' + generateId(),
        type: action.payload.varType || 'float',
        value: action.payload.value !== undefined ? action.payload.value : (action.payload.varType === 'boolean' ? false : (action.payload.varType === 'string' ? '' : 0))
      };
      return pushHistory({
        ...state,
        project: { 
          ...
          settings: { 
            ...project.settings, 
            globalVariables: [...(project.settings.globalVariables || []), newVar] 
          } 
        }
      });
    }
    case 'UPDATE_GLOBAL_VARIABLE': {
      const updatedVars = (project.settings.globalVariables || []).map(v => 
      ...
      return pushHistory({
        ...
        project: { 
          ...project, 
          settings: { ...project.settings, globalVariables: updatedVars } 
        }
      });
    }
    case 'DELETE_GLOBAL_VARIABLE': {
      const updatedVars = (project.settings.globalVariables || []).filter(v => v.id !== action.payload);
      return pushHistory({
        ...state,
        project: { 
          ...project, 
    ...
    case 'ADD_LOCAL_VARIABLE': {
      ...
      const newVar = {
        id: generateId(),
        name: action.payload.name || 'local_' + generateId(),
        type: action.payload.varType || 'float',
        value: action.payload.value !== undefined ? action.payload.value : (action.payload.varType === 'boolean' ? false : (action.payload.varType === 'string' ? '' : 0))
      };
      const updatedScripts = project.scripts.map(s => 
        s.id === scriptId 
          ? { ...s, localVariables: [...(s.localVariables || []), newVar] } 
          : s
      );
      return pushHistory({ ...state, project: { ...project, scripts: updatedScripts } });
    }
    case 'UPDATE_LOCAL_VARIABLE': {
      const { scriptId, varId, updates } = action.payload;
      const updatedScripts = project.scripts.map(s => 
        s.id === scriptId 
          ? { ...s, localVariables: (s.localVariables || []).map(v => v.id === varId ? { ...v, ...updates } : v) } 
          : s
      );
      return pushHistory({ ...state, project: { ...project, scripts: updatedScripts } });
    }
    case 'DELETE_LOCAL_VARIABLE': {
      const { scriptId, varId } = action.payload;
      const updatedScripts = project.scripts.map(s => 
        s.id === scriptId 
          ? { ...s, localVariables: (s.localVariables || []).filter(v => v.id !== varId) } 
          : s
      );
      return pushHistory({ ...state, project: { ...project, scripts: updatedScripts } });
    }
...
const VariableManager = ({ variables, onAdd, onUpdate, onDelete, title = "Variables" }) => {
  ...
  const handleAdd = () => {
    ...
    if (newVarType === 'boolean') {
      parsedValue = newVarValue === 'true' || newVarValue === '1';
    } else if (newVarType === 'float') {
      parsedValue = parseFloat(newVarValue) || 0;
    } else {
      parsedValue = newVarValue;
    }
    onAdd({ name: newVarName.trim(), varType: newVarType, value: parsedValue });
    setNewVarName('');
    setNewVarValue(newVarType === 'boolean' ? 'false' : (newVarType === 'float' ? '0' : ''));
    ...
    <div className="bg-gray-900 border-t border-gray-700">
      ...
      {expanded && (
        <div className="px-3 pb-3 space-y-1">
          {vars.map(v => (
            <div key={v.id} className="flex items-center gap-2 bg-gray-800 rounded px-2 py-1.5">
              <span className="text-[10px] uppercase px-1.5 py-0.5 rounded bg-gray-700 text-gray-400 font-mono">
                {v.type === 'float' ? '#' : v.type === 'string' ? '""' : 'T/F'}
              </span>
              ...
              {editingId === v.id ? (
                <input
                  ...
                  className="w-16 bg-gray-900 border border-gray-600 rounded px-1 py-0.5 text-xs text-gray-200"
                />
              ) : (
                <span 
                  onClick={() => startEdit(v)}
                  className="text-xs text-gray-400 font-mono cursor-pointer hover:text-blue-400 truncate max-w-[80px]"
                >
                  {String(v.value).substring(0, 12)}
                </span>
              )}
              <button onClick={() => onDelete(v.id)} className="text-gray-500 hover:text-red-400">
                <X size={12} />
              </button>
            </div>
          ))}
          ...
          {showAddForm ? (
            <div className="bg-gray-800 rounded p-2 space-y-2 mt-2">
              <input
              ...
              <div className="flex gap-1">
                {['float', 'string', 'boolean'].map(t => (
                  <button
                    key={t}
                    onClick={() => setNewVarType(t)}
                    className={`flex-1 text-[10px] py-1 rounded uppercase font-semibold ${newVarType === t ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-400 hover:bg-gray-600'}`}
                  >
                    {t}
                  </button>
                ))}
              ...
              <input
                ...
                className="w-full bg-gray-900 border border-gray-600 rounded px-2 py-1 text-xs text-gray-200"
                type={newVarType === 'float' ? 'number' : 'text'}
                onKeyDown={e => { if (e.key === 'Enter') handleAdd(); }}
              />
              <div className="flex gap-1">
                <button onClick={() => setShowAddForm(false)} className="flex-1 text-[10px] py-1 rounded bg-gray-700 text-gray-400 hover:bg-gray-600">Cancel</button>
                <button onClick={handleAdd} className="flex-1 text-[10px] py-1 rounded bg-blue-600 text-white hover:bg-blue-500">Add</button>
              </div>
            </div>
          ) : (
            <button 
              onClick={() => setShowAddForm(true)}
              className="w-full flex items-center justify-center gap-1 py-1.5 mt-1 bg-gray-800 hover:bg-gray-700 text-xs text-gray-400 rounded border border-dashed border-gray-600 transition-colors"
            >
...
// ==========================================
// --- EXPRESSION BUILDER SCREEN (RECREATED) ---
// ==========================================

const ExpressionBuilderScreen = () => {
  const { state, dispatch } = useContext(EngineContext);
  const initialExp = state.editor.expressionEditor?.expression;
  const activeScript = state.project.scripts.find(s => s.id === state.editor.activeEditingScriptId);
  ...
  // Parse initial expression to string
  useEffect(() => {
    if (initialExp) {
      if (typeof initialExp === 'object' && initialExp.type) {
        setExpressionString(expressionToString(initialExp));
      } else {
        setExpressionString(String(initialExp));
      }
    } else {
      setExpressionString('');
    }
  ...
  useEffect(() => {
    ...
    try {
      // Simple evaluation for preview (safe subset)
      const safeExpr = expressionString
        .replace(/&&/g, ' && ')
        .replace(/\|\|/g, ' || ')
        .replace(/>/g, ' > ')
        .replace(/</g, ' < ')
      ...
      // Try to evaluate numbers only
      const cleanExpr = safeExpr.replace(/[^0-9\.\+\-\*\/\(\)\s\&\|\>\<\=\!]/g, '');
      if (cleanExpr.trim()) {
  ...
  const expressionToString = (node) => {
    if (!node) return '';
    if (node.type === 'NUMBER' || node.type === 'LITERAL') return String(node.value);
    if (node.type === 'PROPERTY') return String(node.value);
    if (node.type === 'VARIABLE') return String(node.value);
    const opMap = { ADD: '+', SUBTRACT: '-', MULTIPLY: '*', DIVIDE: '/', EQUALS: '==', GREATER_THAN: '>', LESS_THAN: '<', AND: '&&', OR: '||' };
    const left = node.children?.[0] ? expressionToString(node.children[0]) : '';
    const right = node.children?.[1] ? expressionToString(node.children[1]) : '';
    if (left && right) return `(${left} ${opMap[node.type] || ''} ${right})`;
    return '';
  };

  const parseToTree = (str) => {
    // Simple parser - creates a flat tree structure
    const trimmed = str.trim();
    if (!trimmed) return { type: 'LITERAL', value: 0, children: [] };

    // Try parse as number first
    const numVal = parseFloat(trimmed);
    if (!isNaN(numVal) && String(numVal) === trimmed) {
      return { type: 'NUMBER', value: numVal, children: [] };
    }

    // Return as literal string/expression
    return { type: 'LITERAL', value: trimmed, children: [] };
  };

  const insertAtCursor = (text) => {
    const before = expressionString.slice(0, cursorPos);
    const after = expressionString.slice(cursorPos);
    setExpressionString(before + text + after);
    setCursorPos(cursorPos + text.length);
    setTimeout(() => inputRef.current?.focus(), 0);
  };

  const handleSave = () => {
    const tree = parseToTree(expressionString);
    dispatch({ type: 'SAVE_EXPRESSION', payload: { expression: tree } });
  };

  const handleCancel = () => {
    dispatch({ type: 'CLOSE_EXPRESSION_EDITOR' });
  };

  const handleBackspace = () => {
    if (cursorPos > 0) {
      const before = expressionString.slice(0, cursorPos - 1);
      const after = expressionString.slice(cursorPos);
      setExpressionString(before + after);
  ...
  const categories = [
    { 
      id: 'obj_props', 
      label: 'new game object properties', 
      items: ['position.x', 'position.y', 'rotation', 'scale.x', 'scale.y', 'opacity', 'name']
    },
    { 
      id: 'global_props', 
      label: 'global properties', 
      items: ['screen.width', 'screen.height', 'time.delta', 'time.total', 'mouse.x', 'mouse.y']
    },
    { 
      id: 'ui', 
      label: 'ui components', 
      items: ['button.pressed', 'slider.value', 'toggle.state', 'text.content']
    },
    { 
      id: 'sensors', 
      label: 'sensors', 
      items: ['collision.entered', 'collision.exited', 'raycast.hit', 'distance.to']
    },
    { 
      id: 'global_vars', 
      label: 'global variables', 
      items: (state.project.settings.globalVariables || []).map(v => v.name)
    },
    { 
      id: 'local_vars', 
      label: 'local variables', 
      items: (activeScript?.localVariables || []).map(v => v.name)
    },
    { 
      id: 'math', 
      label: 'math', 
      items: ['abs()', 'min()', 'max()', 'sqrt()', 'pow()', 'sin()', 'cos()', 'tan()', 'round()', 'floor()', 'ceil()', 'random()', 'PI', 'E']
    },
  ];
  ...
  const renderKey = (key) => {
    if (key === 'backspace') {
      return (
        <button 
          ...
          className="aspect-square bg-gray-700 hover:bg-gray-600 rounded-lg flex items-center justify-center text-white transition-colors"
        >
          <X size={20} />
        </button>
      );
    ...
    if (key === 'help') {
      return (
        <button 
          key={key}
          onClick={() => alert('Expression Builder Help:\n\nUse numbers, operators, and variables to build expressions.\nClick items from the right panel to insert them.\n\nSupported: + - * / > < == != && || ( )')}
          className="aspect-square bg-gray-700 hover:bg-gray-600 rounded-lg flex items-center justify-center text-yellow-400 transition-colors"
        >
    ...
    return (
      <button 
        ...
        className="aspect-square bg-gray-700 hover:bg-gray-600 rounded-lg flex items-center justify-center text-white text-lg font-medium transition-colors"
      >
        {key}
      </button>
    );
  ...
  return (
    <div className="absolute inset-0 z-[300] bg-gray-800 flex flex-col font-sans text-gray-200">
      {/* Header */}
      <div className="h-14 border-b border-gray-700 bg-gray-900 flex items-center justify-between px-6 shrink-0">
        <div className="flex items-center space-x-3">
          <Variable className="w-5 h-5 text-blue-400" />
          <h2 className="text-lg font-semibold">Expression Editor</h2>
          <span className="text-xs text-gray-500 bg-gray-800 px-2 py-0.5 rounded">{state.editor.expressionEditor?.targetField || 'expression'}</span>
        </div>
        <button onClick={handleCancel} className="p-2 hover:bg-gray-700 rounded-md transition-colors">
          <X className="w-5 h-5" />
        </button>
      </div>

      <div className="flex flex-1 overflow-hidden">
        {/* Left Panel: Expression Input + Numpad */}
        <div className="flex-1 bg-gray-800 p-6 flex flex-col overflow-hidden">
          {/* Expression Input */}
          <div className="mb-4">
            <label className="text-sm font-medium text-gray-300 mb-2 block">Set {state.editor.expressionEditor?.targetField || 'value'}</label>
            <div className="relative">
              <input
                ...
                className="w-full bg-gray-900 border border-gray-600 rounded-lg px-4 py-3 text-lg text-gray-200 focus:outline-none focus:border-blue-500 font-mono"
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <span className="text-sm text-green-400 font-mono">{result || '...'}</span>
            </div>
          </div>
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            <div className="flex space-x-3">
              <button 
                ...
                className="px-6 py-2 rounded-lg font-medium text-sm text-gray-400 hover:text-white hover:bg-gray-700 transition-colors"
              >
                Cancel
              </button>
              <button 
                onClick={handleSave} 
                className="px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-medium text-sm flex items-center gap-2 transition-colors"
              >
        ...
        {/* Right Panel: Categories */}
        <div className="w-72 bg-gray-900 border-l border-gray-700 flex flex-col overflow-hidden shrink-0">
          ...
          <div className="flex-1 overflow-y-auto">
            {categories.map(cat => (
              <div key={cat.id} className="border-b border-gray-700/50">
                <button
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-gray-500 bg-gray-800 px-1.5 py-0.5 rounded">{cat.items.length}</span>
                    {activeCategory === cat.id ? <ChevronDown size={14} className="text-gray-500" /> : <ChevronRight size={14} className="text-gray-500" />}
                  </div>
                </button>
                  ...
                  <div className="px-3 pb-2 space-y-0.5">
                    {cat.items.map((item, idx) => (
                      <button
                        ...
                        className="w-full text-left text-xs text-gray-400 hover:text-blue-400 hover:bg-gray-800 px-2 py-1.5 rounded transition-colors font-mono"
                      >
                        {item}
                      </button>
                    ))}
...
// ==========================================
// --- PERFORMANCE: Asset Map Hook ---
// ==========================================

const useAssetMap = (assets) => {
  return useMemo(() => {
    const map = {};
...
const useDebouncedLocalStorage = (state, key, delay = 2000) => {
...
const runPhysicsSimulation = (scene, dt) => {
  ...
  for (let i = 0; i < objects.length; i++) {
    ...
    for (let j = 0; j < objects.length; j++) {
      ...
      const rotationA = (objA.transform.rotation + (colliderA.rotation || 0)) * (Math.PI / 180);
      const cosA = Math.abs(Math.cos(rotationA));
      const sinA = Math.abs(Math.sin(rotationA));
      const boundWA = halfWA * cosA + halfHA * sinA;
      const boundHA = halfWA * sinA + halfHA * cosA;
      const rectA = {
        minX: objA.transform.x - boundWA + (colliderA.offsetX || 0),
        maxX: objA.transform.x + boundWA + (colliderA.offsetX || 0),
        minY: objA.transform.y - boundHA + (colliderA.offsetY || 0),
        maxY: objA.transform.y + boundHA + (colliderA.offsetY || 0),
      };
      const rotationB = (objB.transform.rotation + (colliderB.rotation || 0)) * (Math.PI / 180);
      const cosB = Math.abs(Math.cos(rotationB));
      const sinB = Math.abs(Math.sin(rotationB));
      const boundWB = halfWB * cosB + halfHB * sinB;
      const boundHB = halfWB * sinB + halfHB * cosB;
      const rectB = {
        minX: objB.transform.x - boundWB + (colliderB.offsetX || 0),
        maxX: objB.transform.x + boundWB + (colliderB.offsetX || 0),
        minY: objB.transform.y - boundHB + (colliderB.offsetY || 0),
        maxY: objB.transform.y + boundHB + (colliderB.offsetY || 0),
      };
...
const TransformGizmo = React.memo(({ object, tool, camera, onInteractionStart }) => {
  ...
  const handleSize = Math.max(36, 24 * invScale);
  const strokeW = Math.max(2, 2 * invScale);
  const hitWidth = Math.max(30, handleSize * 1.5);

  const handleStart = (e, axis) => {
    e.stopPropagation();
    e.preventDefault();
  ...
  return (
    <div className="absolute" style={{ left: `${cx}px`, top: `${cy}px`, transform: `rotate(${rot}deg)`, zIndex: 20, pointerEvents: 'none', touchAction: 'none' }}>
      <svg className="overflow-visible absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" style={{ width: 1, height: 1, pointerEvents: 'none' }}>
        {tool === 'move' && (
          <g style={{ pointerEvents: 'none' }}>
            <circle cx={0} cy={0} r={hitWidth/2} fill="transparent" stroke="transparent" strokeWidth={0}
              style={{ pointerEvents: 'all', cursor: 'move' }}
              onPointerDown={(e) => handleStart(e, 'move_xy')}
              onTouchStart={(e) => handleStart(e, 'move_xy')}
            />
            <circle cx={0} cy={0} r={handleSize/2} fill="#3b82f6" fillOpacity="0.8"
              style={{ pointerEvents: 'none' }}
            />
            <g style={{ pointerEvents: 'none' }}>
              <line x1={0} y1={0} x2={100 * invScale} y2={0} stroke="transparent" strokeWidth={hitWidth}
                style={{ pointerEvents: 'all', cursor: 'ew-resize' }}
                onPointerDown={(e) => handleStart(e, 'move_x')}
                onTouchStart={(e) => handleStart(e, 'move_x')}
              />
              <line x1={0} y1={0} x2={80 * invScale} y2={0} stroke="#ef4444" strokeWidth={strokeW} style={{ pointerEvents: 'none' }} />
              <polygon points={`${80 * invScale},${-6 * invScale} ${100 * invScale},0 ${80 * invScale},${6 * invScale}`} fill="#ef4444" style={{ pointerEvents: 'none' }} />
            </g>
            <g style={{ pointerEvents: 'none' }}>
              <line x1={0} y1={0} x2={0} y2={100 * invScale} stroke="transparent" strokeWidth={hitWidth}
                style={{ pointerEvents: 'all', cursor: 'ns-resize' }}
                onPointerDown={(e) => handleStart(e, 'move_y')}
                onTouchStart={(e) => handleStart(e, 'move_y')}
              />
              <line x1={0} y1={0} x2={0} y2={80 * invScale} stroke="#22c55e" strokeWidth={strokeW} style={{ pointerEvents: 'none' }} />
              <polygon points={`${-6 * invScale},${80 * invScale} 0,${100 * invScale} ${6 * invScale},${80 * invScale}`} fill="#22c55e" style={{ pointerEvents: 'none' }} />
            </g>
          </g>
        )}
        {tool === 'scale' && (
          <g style={{ pointerEvents: 'none' }}>
            ...
            {[
              ...
              <g key={`corner-${i}`} style={{ pointerEvents: 'none' }}>
                <rect x={p.x - hitWidth/2} y={p.y - hitWidth/2} width={hitWidth} height={hitWidth} fill="transparent" stroke="transparent"
                  style={{ pointerEvents: 'all', cursor: p.cursor }}
                  onPointerDown={(e) => handleStart(e, p.axis)}
                  onTouchStart={(e) => handleStart(e, p.axis)}
                />
                <rect x={p.x - handleSize/2} y={p.y - handleSize/2} width={handleSize} height={handleSize} fill="#ffffff" stroke="#3b82f6" strokeWidth={strokeW} style={{ pointerEvents: 'none' }} />
              </g>
            ))}
            {[
              ...
              <g key={`edge-${i}`} style={{ pointerEvents: 'none' }}>
                <rect x={p.x - hitWidth/3} y={p.y - hitWidth/3} width={hitWidth/1.5} height={hitWidth/1.5} fill="transparent" stroke="transparent"
                  style={{ pointerEvents: 'all', cursor: p.cursor }}
                  onPointerDown={(e) => handleStart(e, p.axis)}
                  onTouchStart={(e) => handleStart(e, p.axis)}
                />
                <rect x={p.x - handleSize/3} y={p.y - handleSize/3} width={handleSize/1.5} height={handleSize/1.5} fill="#3b82f6" style={{ pointerEvents: 'none' }} />
              </g>
            ))}
          </g>
        )}
        {tool === 'rotate' && (
          <g style={{ pointerEvents: 'none' }}>
            ...
            <circle cx={0} cy={0} r={80 * invScale + hitWidth/2} fill="none" stroke="transparent" strokeWidth={0}
              style={{ pointerEvents: 'all', cursor: 'grab' }}
              onPointerDown={(e) => handleStart(e, 'rotate')}
              onTouchStart={(e) => handleStart(e, 'rotate')}
            />
            <circle cx={80 * invScale} cy={0} r={handleSize/1.5} fill="#eab308" stroke="#ffffff" strokeWidth={strokeW} style={{ pointerEvents: 'none' }} />
          </g>
        )}
      </svg>
    </div>
...
// ==========================================
// --- GAME FRAME (WITH OFFSET) ---
// ==========================================

const GameFrame = ({ settings, camera }) => {
  const gameFrame = settings.gameFrame || { enabled: true, showInEditor: true, color: '#3b82f6', opacity: 0.3, offsetX: 0, offsetY: 0 };
  ...
  const offsetX = gameFrame.offsetX || 0;
  const offsetY = gameFrame.offsetY || 0;
  ...
  return (
    <div 
      ...
      style={{
        ...
        width: `${vpW}px`,
        height: `${vpH}px`,
        transform: `translate(calc(-50% + ${camera.x + offsetX * camera.zoom}px), calc(-50% + ${camera.y + offsetY * camera.zoom}px)) scale(${camera.zoom})`,
        zIndex: 2,
    ...
    >
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none">
        <div className="absolute top-1/2 left-0 w-full h-px" style={{ backgroundColor: gameFrame.color, opacity: 0.3 }} />
        <div className="absolute left-1/2 top-0 h-full w-px" style={{ backgroundColor: gameFrame.color, opacity: 0.3 }} />
      </div>
    </div>
  );
};
...
const SceneViewport = () => {
  ...
  const assetMap = useAssetMap(state.project.assets);
  const viewportRef = useRef(null);
  const isInteracting = useRef(false);
  const visibleObjects = useMemo(() => {
    return currentScene?.objects.filter(o => !o.hidden && !o._destroyed) || [];
  ...
  const handleTouchStart = (e) => {
    ...
    if (touches.length === 2) {
      ...
      interactionState.current.type = 'pinch_pan';
      ...
      interactionState.current.initialCameraX = camera.x;
      interactionState.current.initialCameraY = camera.y;
      interactionState.current.initialPanCenter = { x: centerX, y: centerY };
      interactionState.current.lastPanCenter = { x: centerX, y: centerY };
      return;
    }
    if (touches.length === 1) {
  ...
  const handleTouchMove = (e) => {
    ...
    if (touches.length === 2 && interactionState.current.isPinching) {
      ...
      const initScreenX = interactionState.current.initialPanCenter.x - rect.left;
      const initScreenY = interactionState.current.initialPanCenter.y - rect.top;
      const vpCenterX = rect.width / 2;
      const vpCenterY = rect.height / 2;
      const worldX = (initScreenX - vpCenterX - interactionState.current.initialCameraX) / interactionState.current.initialZoom;
      const worldY = (initScreenY - vpCenterY - interactionState.current.initialCameraY) / interactionState.current.initialZoom;
      const currScreenX = center.x - rect.left;
      const currScreenY = center.y - rect.top;
      const newCamX = currScreenX - vpCenterX - worldX * newZoom;
      const newCamY = currScreenY - vpCenterY - worldY * newZoom;
      dispatch({ type: 'UPDATE_CAMERA', payload: { zoom: newZoom, x: newCamX, y: newCamY } });
  ...
  const handleTouchEnd = () => {
    ...
    interactionState.current.initialCameraX = null;
    interactionState.current.initialCameraY = null;
  };
  ...
  const handleWheel = (e) => {
    ...
    if (rect) {
      ...
      const worldX = (mouseX - viewportCenterX - camera.x) / camera.zoom;
      const worldY = (mouseY - viewportCenterY - camera.y) / camera.zoom;
      const newX = mouseX - viewportCenterX - worldX * newZoom;
      const newY = mouseY - viewportCenterY - worldY * newZoom;
      dispatch({ type: 'UPDATE_CAMERA', payload: { zoom: newZoom, x: newX, y: newY } });
    } else {
    ...
    >
      ...
      <div className="absolute top-1/2 left-1/2" style={{ transform: `translate(${camera.x}px, ${camera.y}px) scale(${camera.zoom})`, zIndex: 10 }}>
        {visibleObjects.map(obj => {
          ...
          const spriteUrl = obj.rendering?.sprite ? assetMap[obj.rendering.sprite]?.url : null;
          return (
            <div key={obj.id}>
              {hasCollider && (
                <div 
                  ...
                  style={{ 
                    ...
                    transform: `translate(-50%, -50%) rotate(${obj.transform.rotation + (colliderObj?.rotation || 0)}deg)`, 
...
const PropertiesPanel = () => {
  ...
  const spriteAsset = currentSprite ? assetMap[currentSprite] : null;
      ...
      <div className="flex-1 overflow-y-auto pb-10">
          ...
          <div className="relative mt-2">
            ...
            {showBehaviorMenu && (
              <div className="absolute top-full left-0 right-0 mt-1 bg-gray-800 border border-gray-700 rounded-lg shadow-xl z-[100] overflow-hidden">
                ...
                <button onClick={() => { dispatch({ type: 'ADD_BEHAVIOR', payload: { objectId: selectedObj.id, behaviorType: 'BoxCollider', behaviorData: { width: selectedObj.transform.w, height: selectedObj.transform.h, offsetX: 0, offsetY: 0, rotation: 0 } } }); setShowBehaviorMenu(false); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-300 hover:bg-gray-700 hover:text-white transition-colors">
        ...
        <PanelSection title="Visual Scripts" icon={Code}>
          {attachedScripts.map(s => (
            <ScriptItem key={s.id} scriptRef={s} objectId={selectedObj.id} />
          ))}
          <div className="relative">
            ...
            {showScriptMenu && (
              <div className="absolute top-full left-0 right-0 mt-1 bg-gray-800 border border-gray-700 rounded-lg shadow-xl z-[100] overflow-hidden max-h-48 overflow-y-auto">
                <button onClick={() => { dispatch({ type: 'ADD_SCRIPT', payload: { name: 'New Script', nodes: [], connections: [] } }); setShowScriptMenu(false); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-purple-300 hover:bg-gray-700 hover:text-white transition-colors border-b border-gray-700">
                  <Plus size={14} className="text-purple-400" /> <span className="font-semibold">New Script</span>
                </button>
                {availableScripts.length === 0 && <div className="px-3 py-2 text-sm text-gray-500">No existing scripts</div>}
...
// ==========================================
// --- SCRIPT ITEM & BEHAVIOR ITEM ---
// ==========================================

const ScriptItem = ({ scriptRef, objectId }) => {
  const { state, dispatch } = useContext(EngineContext);
  const [expanded, setExpanded] = useState(false);
  const [isRenaming, setIsRenaming] = useState(false);
  const [editName, setEditName] = useState(scriptRef.name);
  const script = state.project.scripts.find(s => s.id === scriptRef.scriptId);

  const handleRename = () => {
    if (editName.trim()) {
      dispatch({ type: 'RENAME_SCRIPT', payload: { id: scriptRef.scriptId, name: editName.trim() } });
    }
    setIsRenaming(false);
  };
  ...
  return (
    <div className="bg-gray-900 border border-gray-700 rounded mb-2 overflow-hidden">
      <div className="flex items-center justify-between p-2 hover:bg-gray-800 transition-colors">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <button 
            onClick={() => dispatch({ type: 'TOGGLE_OBJECT_SCRIPT', payload: { objectId, scriptRefId: scriptRef.id } })}
            className={`w-4 h-4 rounded border shrink-0 ${scriptRef.enabled ? 'bg-green-500 border-green-500' : 'border-gray-500'}`}
          />
          {isRenaming ? (
            <input
              autoFocus
              ...
              onBlur={handleRename}
              onKeyDown={e => { if (e.key === 'Enter') handleRename(); if (e.key === 'Escape') { setEditName(scriptRef.name); setIsRenaming(false); }}}
              className="flex-1 min-w-0 bg-gray-800 border border-gray-600 rounded px-1.5 py-0.5 text-xs text-gray-200"
            />
          ) : (
            <span 
              onClick={() => { setEditName(scriptRef.name); setIsRenaming(true); }}
              className={`text-xs truncate cursor-pointer hover:text-blue-400 transition-colors ${scriptRef.enabled ? 'text-gray-300' : 'text-gray-500'}`}
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <button 
            onClick={() => dispatch({ type: 'SET_EDITING_SCRIPT', payload: scriptRef.scriptId })}
            className="bg-purple-600 hover:bg-purple-500 text-white px-2 py-0.5 rounded text-[10px] font-bold tracking-wide"
          >
            Open Editor
          </button>
          <Trash2 
            ...
            onClick={() => dispatch({ type: 'REMOVE_OBJECT_SCRIPT', payload: { objectId, scriptRefId: scriptRef.id } })} 
          />
        </div>
      </div>
      {expanded && script && (
        <div className="px-2 pb-2 border-t border-gray-700/50">
          <div className="mt-1.5 space-y-1">
            <div className="text-[10px] text-gray-500 flex items-center gap-1">
              <Code size={10} /> {script.nodes?.length || 0} nodes · {script.connections?.length || 0} connections
            </div>
            <div className="flex flex-wrap gap-1">
              {script.nodes?.slice(0, 3).map((node, i) => (
                <span key={i} className="text-[10px] px-1.5 py-0.5 bg-gray-800 rounded text-gray-400 border border-gray-700">
                  {node.name}
                </span>
              ))}
              {(script.nodes?.length || 0) > 3 && (
                <span className="text-[10px] px-1.5 py-0.5 bg-gray-800 rounded text-gray-500">+{(script.nodes?.length || 0) - 3} more</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="p-2 border-t border-gray-700">
          ...
          {behavior.type === 'BoxCollider' && (
            <>
              ...
              <MobileInput label="Rotation" value={behavior.rotation || 0} onChange={v => updateBehavior({ rotation: v })} type="number" />
            </>
          )}
...
// ==========================================
// --- VISUAL SCRIPTING EDITOR (FIXED DYNAMIC CONNECTIONS) ---
// ==========================================
...
const VisualScriptingScreen = () => {
  ...
  // Dynamic connection lines - FIXED: Lines now render inside the transformed container
  // so they scale/zoom correctly with the camera
  ...
  const handleCanvasPointerDown = (e) => {
    ...
    if (nearbyPort && !nearbyPort.isOutput && connectingFrom) {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-900 w-full relative select-none">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        >
          {/* 
            FIXED DYNAMIC CONNECTIONS:
            The SVG is now placed INSIDE the same transformed container as the nodes.
            This means connections scale, pan, and zoom perfectly with the camera.
          */}
          <div 
          ...
          {/* Nodes Layer - Same transform as connections */}
          <div 
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <X size={12} className="cursor-pointer hover:text-red-300 opacity-70 hover:opacity-100" onClick={(e) => { e.stopPropagation(); deleteNode(node.id); }} />
                  </div>
                          ...
                          {Object.entries(node.params).map(([key, val]) => (
                            <div key={key} className="flex items-center gap-1">
                              ...
                              <input 
                                ...
                                value={typeof val === 'object' ? '[Expression]' : val} 
                                onChange={(e) => updateNodeParam(node.id, key, typeof val === 'number' ? parseFloat(e.target.value) || 0 : e.target.value)} 
                                onClick={(e) => e.stopPropagation()} 
                                disabled={typeof val === 'object'}
                                className={`flex-1 bg-gray-900 border border-gray-600 rounded px-1 py-0.5 text-xs text-gray-200 ${typeof val === 'object' ? 'opacity-50 cursor-not-allowed' : ''}`}
                                style={{ fontSize: '12px' }} 
                              />
                            </div>
                          ))}
                  ...
                  <div 
                    className="absolute top-1/2 -right-2 w-4 h-4 rounded-full border-2 border-gray-800 transform -translate-y-1/2 cursor-crosshair transition-transform hover:scale-125"
                    style={{ 
                  ...
                  {!isEvent && (
                    <div 
                      className="absolute top-1/2 -left-2 w-4 h-4 rounded-full border-2 border-gray-800 transform -translate-y-1/2 cursor-crosshair transition-transform hover:scale-125"
                      style={{ 
                        backgroundColor: dataTypeColor,
                        boxShadow: (() => {
                      ...
                      <div key={key}>
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <button 
                            onClick={() => {
                              dispatch({ 
                                type: 'OPEN_EXPRESSION_EDITOR', 
                                payload: { nodeId: node.id, field: key, initialExpression: val } 
                              });
                              setEditingNode(null);
                            }}
                            className="text-[10px] text-purple-400 hover:text-purple-300 flex items-center gap-1 px-1.5 py-0.5 rounded hover:bg-purple-500/10 transition-colors"
                          >
                            <Code size={10} /> Open Expression Builder
                          </button>
                        </div>
                        <input 
                          type={typeof val === 'number' ? 'number' : 'text'}
                          value={typeof val === 'object' ? '[Expression Tree]' : val}
                          onChange={(e) => updateNodeParam(node.id, key, typeof val === 'number' ? parseFloat(e.target.value) || 0 : e.target.value)}
                          disabled={typeof val === 'object'}
                          className={`w-full bg-gray-900 border border-gray-600 rounded px-3 py-2 text-sm text-gray-200 focus:outline-none focus:border-purple-500 ${typeof val === 'object' ? 'opacity-50 cursor-not-allowed' : ''}`}
                        />
                      </div>
                    ))}
                  </div>
      ...
      {/* Local Variables Panel (Bottom) */}
      {showLocalVars && activeScript && (
        <div className="border-t border-gray-700 bg-gray-850 shrink-0">
          <VariableManager
            title="Local Variables"
            variables={activeScript.localVariables || []}
            onAdd={(data) => dispatch({ type: 'ADD_LOCAL_VARIABLE', payload: { scriptId: activeScript.id, ...data } })}
            onUpdate={(id, updates) => dispatch({ type: 'UPDATE_LOCAL_VARIABLE', payload: { scriptId: activeScript.id, varId: id, updates } })}
            onDelete={(id) => dispatch({ type: 'DELETE_LOCAL_VARIABLE', payload: { scriptId: activeScript.id, varId: id } })}
          />
        </div>
      )}
    </div>
...
const ProjectManagerScreen = () => {
  ...
  const handleImport = (e) => {
    ...
    reader.onerror = () => { alert('Failed to read project file'); };
...
// ==========================================
// --- SETTINGS SCREEN (WITH GLOBAL VARIABLES) ---
// ==========================================
    ...
    <div className="flex flex-col h-full bg-gray-900 w-full select-none">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          <div className="bg-gray-850 border border-gray-800 rounded-xl p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-4 mt-3">
                <MobileInput label="Offset X" value={settings.gameFrame?.offsetX || 0} onChange={v => updateGameFrame('offsetX', v)} type="number" />
                <MobileInput label="Offset Y" value={settings.gameFrame?.offsetY || 0} onChange={v => updateGameFrame('offsetY', v)} type="number" />
              </div>
          ...
          <div className="bg-gray-850 border border-gray-800 rounded-xl p-6">
            <h3 className="text-lg font-semibold text-gray-200 mb-4 flex items-center gap-2"><Globe size={20} className="text-yellow-400" /> Global Variables</h3>
            <VariableManager
              title="Global Variables"
              variables={settings.globalVariables || []}
              onAdd={(data) => dispatch({ type: 'ADD_GLOBAL_VARIABLE', payload: data })}
              onUpdate={(id, updates) => dispatch({ type: 'UPDATE_GLOBAL_VARIABLE', payload: { id, updates } })}
              onDelete={(id) => dispatch({ type: 'DELETE_GLOBAL_VARIABLE', payload: id })}
            />
          </div>
...
const FullscreenPlayMode = ({ onStop }) => {
  ...
  const assetMap = useAssetMap(state.project.assets);
  const visibleObjects = useMemo(() => {
    return currentScene?.objects.filter(o => !o.hidden && o.type !== 'Camera') || [];
  }, [currentScene?.objects]);
...
const SceneEditorScreen = () => {
  ...
  // Default camera at 0,0 (center of viewport)
  useEffect(() => {
    dispatch({ 
      type: 'UPDATE_CAMERA', 
      payload: { x: 0, y: 0, zoom: 0.6 } 
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const isPlayingRef = useRef(state.editor.isPlaying);
  const activeSceneIdRef = useRef(state.editor.activeSceneId);
  const projectRef = useRef(state.project);
  const playTimeRef = useRef(state.editor.playTime);

  useEffect(() => { isPlayingRef.current = state.editor.isPlaying; }, [state.editor.isPlaying]);
  useEffect(() => { activeSceneIdRef.current = state.editor.activeSceneId; }, [state.editor.activeSceneId]);
  useEffect(() => { projectRef.current = state.project; }, [state.project]);
  useEffect(() => { playTimeRef.current = state.editor.playTime; }, [state.editor.playTime]);
  ...
  useEffect(() => {
    ...
    const loop = (timestamp) => {
      ...
      if (isPlayingRef.current) {
        ...
        const updatedScene = runPhysicsSimulation(currentScene, 1/60);
        const scenesList = currentProject.scenes.map(s => s.id === currentSceneId ? updatedScene : s);
        const newProject = executeScripts({ ...currentProject, scenes: scenesList });
        dispatch({ type: 'APPLY_SCRIPT_CHANGES', payload: { scenes: newProject.scenes } });
        dispatch({ type: 'UPDATE_PLAY_TIME', payload: playTimeRef.current + 1/60 });
      }
    };
    frameId = requestAnimationFrame(loop);
  ...
  useDebouncedLocalStorage(state, 'nebula_engine_state_v4', 2000);

  const recenterCamera = () => {
    dispatch({ type: 'UPDATE_CAMERA', payload: { x: 0, y: 0, zoom: 0.6 } });
  };
...
export default function App() {
  ...
  return (
    <EngineContext.Provider value={{ state, dispatch }}>
      <div className="h-screen w-screen bg-black overflow-hidden flex font-sans text-gray-200 relative">
         {renderScreen()}
         {state.editor.expressionEditor?.isOpen && (
           <ExpressionBuilderScreen />
         )}
      </div>
